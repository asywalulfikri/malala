...
public class MainActivity extends FragmentActivity implements
	...
	public class Data extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... arg0) {
	        ...
	        url = alamat.URl_GET_MAPS +idwisata;
			...
			try {
				college = json.getJSONArray("malala");
				...
				for (int i = 0; i <= college.length(); i++) {
					...
					String id_1 = c.getString("id_wisata").trim();
					String latitude_1 = c.getString("latitude").trim();
					String longitude_1 = c.getString("longitude").trim();
					String nama_1 = c.getString("nama_tempat_wisata").trim();
					
					map.put("id_wisata", id_1);
					map.put("nama_tempat_wisata", nama_1);
		...
		protected void onPostExecute(String result) {
			...
			for (int x = 0; x < dataList.size(); x = x + 1) {
				...
				String nama = dataList.get(x).get("nama_tempat_wisata");
			
				googleMap.addMarker(new MarkerOptions()
						.position(posisi)
						.title(nama)
						.icon(BitmapDescriptorFactory
								.fromResource(R.drawable.mapsmm)));
